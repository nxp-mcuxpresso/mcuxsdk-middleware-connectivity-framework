...
secResultType_t SecLibMutexCreate(void)
{
    ...
    if (!seclib_mutex_created)
    {
        ...
        osa_status_t ret = OSA_MutexCreate((osa_mutex_handle_t)mSecLibSssMutexId);
...
{
#if gSecLibUseMutex_c
    osa_status_t ret = OSA_MutexLock((osa_mutex_handle_t)mSecLibSssMutexId, osaWaitForever_c);
...
{
#if gSecLibUseMutex_c
    osa_status_t ret = OSA_MutexUnlock((osa_mutex_handle_t)mSecLibSssMutexId);
    return (ret == KOSA_StatusSuccess) ? gSecSuccess_c : gSecError_c;
#else
    return gSecSuccess_c;
#endif
